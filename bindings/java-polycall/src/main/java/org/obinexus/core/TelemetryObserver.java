...
public class TelemetryObserver {
    ...
    public enum ConsensusState {
        ...
        public static ConsensusState fromCliValue(String value) {
            if (value == null || value.isBlank()) {
                throw new IllegalArgumentException("Consensus state is required (yes|no|maybe)");
            }
            return ConsensusState.valueOf(value.trim().toUpperCase());
        }
    }
    ...
    public void recordConsensus(ConsensusState state, Map<String, Object> payload) {
        recordEvent("telemetry.consensus", payload);
        metrics.put("telemetry.consensus.state", state.name());
        logger.info("Recorded trinary consensus state={} session={}", state.name(), payload.get("session_id"));
    }
